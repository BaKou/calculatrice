...
describe('CardsService', () => {
  ...
  it('should reset player and bank card when start a new game', (done) => {
    ...
    service.playerCards$.subscribe(playerCard => { expect(playerCard).toEqual([]) })
    service.bankCards$.subscribe(bankCard => { expect(bankCard).toEqual([]) })
    done();
  });
  ...
  it('should initialise the game and give player and bank card', (done) => {
    ...
    service.startGame();
    httpController.expectOne(`https://deckofcardsapi.com/api/deck/new/shuffle/?deck_count=1`)
      .flush(mockDeckResponse);

    expect(deckSpy).toBeCalled();

    httpController.match(`https://deckofcardsapi.com/api/deck/${deckId}/draw/?2`);

    expect(cardSpy).toBeCalledTimes(2);

    service.bankCards$.subscribe(cards => expect(cards).toStrictEqual(mockCardResponse.cards));
  ...
  it('should get one card and update player card when player draw during the game', async () => {
    ...
    httpController.expectOne(`https://deckofcardsapi.com/api/deck/${deckId}/draw/?1`).flush(mockCardResponseTwo);

    service.playerCards$.subscribe(bc => { expect(bc).toEqual(mockCardResponse.cards) })
    service.bankCards$.subscribe(bc => { expect(bc).toEqual([]) })
  });
});
